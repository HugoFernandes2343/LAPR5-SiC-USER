...
public class Graph<V, E> implements GraphInterface<V, E> {
    ...
    public Iterable<Edge<V, E>> edges() {
        ArrayList<Edge<V, E>> edgs = new ArrayList<>();

        for (Map.Entry<V, Vertex<V, E>> entry : vertices.entrySet()) {
            Iterable<Edge<V, E>> itr = entry.getValue().getAllOutEdges();
            for (Edge<V, E> a : itr) {
                edgs.add(a);
            }
        }
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        ArrayList<Edge<V, E>> incEdges = new ArrayList<>();
        ...
        for (Vertex<V, E> vertex : vertices.values()) {
            if (vertex.getEdge(vert) != null) {
                incEdges.add(vertex.getEdge(vert));
            }
        }
